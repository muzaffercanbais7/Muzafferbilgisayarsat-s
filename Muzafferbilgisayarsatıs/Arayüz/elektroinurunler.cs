...
namespace Muzafferbilgisayarsatıs.UI
{
    public partial class Ürünler : Form
    {
        ...
        tekrar:
            ...
            {
                bool b = ayrıntı.UrunEkle(frm.Urun);

                if (b)
                {

                    DataSet ds = ayrıntı.UrunGetir("");
                    if (ds != null)
        ...
        {
            DataSet ds = ayrıntı.UrunGetir(toolStripTextBox2.Text);
            if (ds != null)
                dataGridView2.DataSource = ds.Tables[0];
        }
        ...
        private void btnÜrünDüzenle_Click(object sender, EventArgs e)
        {
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = ayrıntı.UrunGüncelle(frm.Urun);
